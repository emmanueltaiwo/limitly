...
interface LimitlyConfig {
  ...
  servicePassword?: string;  // Password for collision detection (only used when redisUrl is not provided)
...
- **Without `redisUrl`**: Uses HTTP API mode (hosted service). Use `servicePassword` to prevent collisions with other users using the same `serviceId`.
...
## Service Password (Collision Prevention)

When using HTTP API mode (without `redisUrl`), use `servicePassword` to prevent collisions with other users:
...
**How it works:**
- First user to use a `serviceId` with a password "claims" it
- Subsequent users must provide the same password
- Password mismatch triggers a warning (request still allowed)
- Passwords are hashed and stored securely

**Benefits:**
- Prevents accidental collisions with other users
- No need for unique serviceIds
- Works seamlessly with HTTP API mode

**Note:** If you provide `redisUrl`, `servicePassword` is ignored (not needed since you own your Redis).
...
**Privacy:**
- All identifiers (service IDs, client IDs) are hashed before sending
